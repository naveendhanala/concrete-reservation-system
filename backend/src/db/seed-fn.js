// Reusable seed function (used by seed.js and /api/seed endpoint)
...
async function seed() {
  ...
  try {
    ...
    for (const pkgId of ch1Packages) await client.query('INSERT INTO user_packages VALUES ($1,$2) ON CONFLICT DO NOTHING', [ch1Id, pkgId]);
    for (const pkgId of ch2Packages) await client.query('INSERT INTO user_packages VALUES ($1,$2) ON CONFLICT DO NOTHING', [ch2Id, pkgId]);
    ...
    await client.query('INSERT INTO user_batching_plants VALUES ($1,$2) ON CONFLICT DO NOTHING', [pmm1Id, plantIds['Batching Plant 1']]);
    await client.query('INSERT INTO user_batching_plants VALUES ($1,$2) ON CONFLICT DO NOTHING', [pmm2Id, plantIds['Batching Plant 2']]);
    await client.query('INSERT INTO user_batching_plants VALUES ($1,$2) ON CONFLICT DO NOTHING', [pmm3Id, plantIds['Batching Plant 3']]);

    for (let i = 0; i < packages.length; i++) {
      const pmId = await insertUser(client, {
        name: `Project Manager ${i + 1}`, role: 'PM',
        email: `pm${i + 1}@concrete.com`, hash: hash('PM@123'),
      });
      await client.query('INSERT INTO user_packages VALUES ($1,$2) ON CONFLICT DO NOTHING', [pmId, pkgIds[packages[i]]]);
    }

    await client.query(`DELETE FROM site_engineers WHERE package_id = ANY($1)
      AND engineer_id NOT IN (SELECT site_engineer_id FROM reservations WHERE site_engineer_id IS NOT NULL)`,
    ...
    for (const name of contractors) {
      await client.query(`INSERT INTO contractors (name) VALUES ($1) ON CONFLICT DO NOTHING`, [name]);
    }
    ...
    for (let day = 0; day < 14; day++) {
      ...
      for (const slot of generateSlotsForDate(`${yyyy}-${mm}-${dd}`)) {
        await client.query(
